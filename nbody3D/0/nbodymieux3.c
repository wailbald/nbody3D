...
void move_particles(particle_t *p, f32 dt, u64 n)
{
  ...
  for (u64 i = 0; i < n; i+=8)
  {
    ...
    for (u64 j = 0; j < n; j++)
    {
      ...
      tmp = _mm256_mul_ps(dx,d_3_over_2);
      fx = _mm256_add_ps(fx,tmp);

      tmp = _mm256_mul_ps(dy,d_3_over_2);
      fy = _mm256_add_ps(fy,tmp);

      tmp = _mm256_mul_ps(dz,d_3_over_2);
      fz = _mm256_add_ps(fz,tmp);
    }

    //
    //vx = _mm256_fmadd_ps(mdt,fx,vx);
    tmp = _mm256_mul_ps(mdt,fx);
    vx = _mm256_add_ps(vx,tmp);

    //vy = _mm256_fmadd_ps(mdt,fy,vy);
    tmp = _mm256_mul_ps(mdt,fy);
    vy = _mm256_add_ps(vy,tmp);

    //vz = _mm256_fmadd_ps(mdt,fz,vz);
    tmp = _mm256_mul_ps(mdt,fz);
    vz = _mm256_add_ps(vz,tmp);
  ...
  for (u64 i = 0; i < n; i+=8)
    {
      ...
      tmp = _mm256_mul_ps(mdt,vx);
      x1 = _mm256_add_ps(tmp,x1);

      tmp = _mm256_mul_ps(mdt,vy);
      y1 = _mm256_add_ps(tmp,y1);

      tmp = _mm256_mul_ps(mdt,vz);
      z1 = _mm256_add_ps(tmp,z1);
...
int main(int argc, char **argv)
{
  ...
  p->x = aligned_alloc(64,sizeof(f32) * (n+7));
  p->y = aligned_alloc(64,sizeof(f32) * (n+7));
  p->z = aligned_alloc(64,sizeof(f32) * (n+7));

  p->vx = aligned_alloc(64,sizeof(f32) * (n+7));
  p->vy = aligned_alloc(64,sizeof(f32) * (n+7));
  p->vz = aligned_alloc(64,sizeof(f32) * (n+7));
